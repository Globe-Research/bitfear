...
def get_instruments_with_expiry(expiry, currency='BTC', kind='option', expired='false'):
    try:
        ...
        return [instrument['instrument_name'] for instrument in instrument_result if
                expiry in instrument['instrument_name']]
    except dbitApi.exceptions.ApiException as e:
        print(e)
        # logger.exception('Exception when calling MarketDataApi->public_get_instruments_get!')
        exit()
...
def get_consecutive_zeroes(zero_bid):

    zero_bid_temp = zero_bid.copy()
    zero_bid = np.array(zero_bid)
    zero_bid_temp = np.array(zero_bid_temp)
    zero_bid_temp = np.delete(zero_bid_temp, 0)
    zero_bid_temp = np.append(zero_bid_temp, [0])

    return pd.Series(zero_bid_temp * zero_bid).cumsum()


def calculate_indices(time, near_datetime, next_datetime, const_mature_days, R, near_calls_df, near_puts_df,
                      next_calls_df, next_puts_df):
    ...
    # Added 16 hours to the times to account for 4pm expiration time ToDo

    n1 = (near_datetime - time).total_seconds()/ 60 + 16 * 60
    n2 = (next_datetime - time).total_seconds() / 60 + 60 * 60
    nY = 525600
    ...
    # changed the way the consecutive zero bids were calculated ToDo
    ...
    near_otm_puts_df['zero_bid_cumsum'] = get_consecutive_zeroes(near_otm_puts_df['zero_bid'])
    near_otm_puts_df = near_otm_puts_df[(near_otm_puts_df['zero_bid_cumsum'] <= 0) & (near_otm_puts_df['best_bid'] > 0)]

    near_otm_calls_df = near_otm_calls_df.assign(zero_bid=lambda df: (df['best_bid'] == 0).astype(int))
    near_otm_calls_df['zero_bid_cumsum'] = get_consecutive_zeroes(near_otm_calls_df['zero_bid'])
    near_otm_calls_df = near_otm_calls_df[
        (near_otm_calls_df['zero_bid_cumsum'] <= 0) & (near_otm_calls_df['best_bid'] > 0)]
    ...
    next_otm_puts_df['zero_bid_cumsum'] = get_consecutive_zeroes(next_otm_puts_df['zero_bid'])
    next_otm_puts_df = next_otm_puts_df[(next_otm_puts_df['zero_bid_cumsum'] <= 0) & (next_otm_puts_df['best_bid'] > 0)]

    next_otm_calls_df = next_otm_calls_df.assign(zero_bid=lambda df: (df['best_bid'] == 0).astype(int))
    next_otm_calls_df['zero_bid_cumsum'] = get_consecutive_zeroes(next_otm_calls_df['zero_bid'])
    next_otm_calls_df = next_otm_calls_df[
        (next_otm_calls_df['zero_bid_cumsum'] <= 0) & (next_otm_calls_df['best_bid'] > 0)]
    ...
    # changed the calculation of vix ToDo

    VXBT = 100 * np.sqrt(omega * t1 * sigma1 * (nY / n) + (1 - omega) * t2 * sigma2 * (nY / n))
    GVXBT = np.sqrt(omega * t1 * sigma1 * n + (1 - omega) * t2 * sigma2 * n)
    AVXBT = np.sqrt(omega * t1 * sigma1_a * n + (1 - omega) * t2 * sigma2_a * n)
...
def get_indices(maturity=7, rate=0, live=True, time=None, dfs=None):
    if live:
        ...
        VXBT, GVXBT, AVXBT = calculate_indices(now, near_datetime, next_datetime, maturity, rate, near_calls_df,
                                               near_puts_df, next_calls_df, next_puts_df)

    else:
        near_expiry, next_expiry, near_datetime, next_datetime = get_near_next_terms(time)
        VXBT, GVXBT, AVXBT = calculate_indices(time, near_datetime, next_datetime, maturity, rate, dfs[0], dfs[1],
                                               dfs[2], dfs[3])
